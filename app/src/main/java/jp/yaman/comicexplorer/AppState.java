...
public final class AppState {
    ...
    public static final class SavedItem {
        ...
        SavedItem(Uri uri, String title, String kind, long timestamp) {
            this.uri = uri;
            this.title = title;
            this.kind = kind;
    ...
    public static void setBookmark(Context context, Uri uri, int page, boolean bookmarked) {
        ...
        prefs(context).edit().putStringSet("bookmark." + key(uri), values).apply();
    }

    public static void clearBookmarks(Context context, Uri uri) {
        prefs(context).edit().remove("bookmark." + key(uri)).apply();
    }
    ...
    public static boolean startFullscreen(Context context) {
        return prefs(context).getBoolean("setting.start_fullscreen", false);
    }
    ...
    public static void clearReadingData(Context context) {
        ...
        for (String name : pref.getAll().keySet()) {
            if (name.startsWith("position.") || name.startsWith("bookmark.")) editor.remove(name);
        }
        editor.apply();
    }
